...
def prepareSlotValuesIndependent(data_name):
    ...
    db_pattern = DATA_DIR + data_name + '/{}_db.json'
    # read databases
    for domain in domains:
        try:
            fin = file(db_pattern.format(domain))
    ...
    fin = open(os.path.join(DATA_DIR, data_name+'/db/train_db.json'))
    db_json = json.load(fin)
    fin.close()
